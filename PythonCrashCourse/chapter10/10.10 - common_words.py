...
filenames = ['a_room_with_a_view.txt', 'middlemarch.txt', 'little_women.txt']

for filename in filenames:
    path = Path(filename)
    full_text = path.read_text(encoding='utf-8')
    chosen_word_count = full_text.count('the')
    print(f"{filename} had about {chosen_word_count} amount of the word 'the'.")
